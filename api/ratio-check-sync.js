...
export default async function handler(req, res) {
  ...
  for (const [empIdStr, newCoverType] of Object.entries(coverTypeByEmp)) {
    ...
    const patchedSchedule = schedule.map(block => {
      ...
      // Only patch blocks that were generated by this sync (ratio/programming
      // cover). Leave lunch/cleaning/additional/own-lunch blocks planned in the
      // Float Schedule panel untouched so they keep their intended coverType.
      const ratioCheckNotes = ['Ratio cover', 'Programming cover'];
      if (!ratioCheckNotes.includes(block.notes)) return block;
