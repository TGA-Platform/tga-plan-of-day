 ...
 *
 * Fetches Deputy rosters for the next 7 days (no unit filter — catches every centre)
 ...
 * With this cron running every 5 minutes, the cache is always fresh for the whole week:
 * - New starters, sick day removals, shift swaps, future roster updates → reflected within 5 minutes
...
function getNextWeekDatesSydney() {
  const today = getTodaySydney();
  const dates = [];
  for (let i = 0; i < 7; i++) dates.push(addDaysSydney(today, i));
  return dates;
}
...
export default async function handler(req, res) {
  ...
  try {
    const dates = getNextWeekDatesSydney();
    let totalRosters = 0;
    ...
    console.log(`[cron-roster-refresh] ${dates.join(',')}: ${totalRosters} rosters, ${ms}ms`);
    return res.status(200).json({ ok: true, dates, count: totalRosters, ms });
  } catch (err) {
