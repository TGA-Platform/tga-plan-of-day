 ...
 * /api/staff-accidents
 *
 * GET  ?centreId=bexley               → all accidents for a centre (or all if no centreId)
 * GET  ?all=true                      → all accidents across all centres
    ...
    try {
      switch (action) {
        case 'create': {
          ...
          const [created] = await sbPost('/staff_accidents', {
            ...fields,
        ...
        case 'update': {
          ...
          const [updated] = await sbPatch(`/staff_accidents?id=eq.${id}`, {
            ...fields,
