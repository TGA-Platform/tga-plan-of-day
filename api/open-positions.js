 ...
 * /api/open-positions
 *
 * GET  ?centreId=bexley               → all open positions for a centre
 * POST body { action, ...params }
 *
 * Actions:
 *   create  { centre_id, title, qualification_required, room_id?, status, notes? }
 *   update  { id, ...fields }
 *   delete  { id }
 */
...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    try {
      ...
      let path = '/open_positions?order=created_at.desc';
      if (centreId) path += `&centre_id=eq.${centreId}`;
      const data = await sbGet(path);
      return res.json(data);
    } catch (err) {
      if (err.message && err.message.includes('42P01')) return res.json([]);
    ...
    try {
      switch (action) {
        case 'create': {
          ...
          if (!fields.title || !fields.centre_id) return res.status(400).json({ error: 'centre_id and title required' });
          const [created] = await sbPost('/open_positions', { ...fields, created_at: new Date().toISOString() });
          return res.json({ ok: true, data: created });
        }
        case 'update': {
          const { action: _a, id, ...fields } = body;
          if (!id) return res.status(400).json({ error: 'id required' });
          const [updated] = await sbPatch(`/open_positions?id=eq.${id}`, { ...fields, updated_at: new Date().toISOString() });
