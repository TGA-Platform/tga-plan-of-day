...
/** Build a fallback lunch schedule from float_schedules coverage blocks.
 *  Used when no saved lunch_schedules row exists yet, so both the Ratio Check
 *  grid and the Lunch Break panel show the planned covers. */
function buildLunchScheduleFromFloats(rosters: RosteredStaff[], floatScheds: any[]): LunchBreakEntry[] {
  const rosterById = new Map<number, RosteredStaff>();
  for (const r of rosters) {
    if (!rosterById.has(r.employeeId)) rosterById.set(r.employeeId, r);
  }

  const byEmployee = new Map<number, LunchBreakEntry>();
  for (const fsRow of floatScheds) {
    for (const block of (fsRow.schedule ?? [])) {
      ...
      byEmployee.set(empId, {
        ...
        room: staff.unitName || '',
        shiftStart: formatRosterTime(staff.startTime),
        shiftEnd: formatRosterTime(staff.endTime),
        lunchStart: String(block.startTime ?? ''),
        lunchEnd: String(block.endTime ?? ''),
        coveredBy: coveredId
          ? { employeeId: fsRow.employee_id, name: fsRow.employee_name, type: 'float' }
          : null,
        needsCover: false,
        autoGenerated: true,
      });
...
export default function RatioCheckPanel({ centreId, date, rooms, children, rosters,
  ...
  // Track whether we've already attempted to backfill lunch_schedules from
  // float_schedules for the current centre/date, so we don't loop or spam saves.
  const lunchBackfillAttempted = useRef(false);
  ...
  const scheduledLunchByEmployee = useMemo(() => {
    ...
    if (Object.keys(map).length === 0) {
      for (const entry of buildLunchScheduleFromFloats(rosters, floatScheds)) {
        map[entry.employeeId] = { lunchStart: entry.lunchStart, lunchEnd: entry.lunchEnd };
      }
    }
    return map;
  ...
  useEffect(() => {
    ...
    if (lunchSchedule.length > 0) { lunchBackfillAttempted.current = true; return; }
    if (floatScheds.length === 0 || rosters.length === 0) return;

    lunchBackfillAttempted.current = true;
  ...
  useEffect(() => {
    lunchBackfillAttempted.current = false;
  }, [centreId, date]);
  ...
  const staffAtSlotMap = useMemo(() => {
    ...
    for (const fsRow of floatScheds) {
      ...
      console.warn(`[RatioCheck] ${centreId} ${date}: float_schedules staff ${fsRow.employee_name} (${empId}) missing from Deputy rosters — using synthetic roster. If this persists, check /api/deputy-rosters filtering.`);
